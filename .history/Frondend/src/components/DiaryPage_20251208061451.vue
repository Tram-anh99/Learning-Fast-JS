<template>
  <div class="diary-container bg-gray-100 min-h-screen pb-20">
    <div v-if="mode === 'list'">
      ...
      <div class="p-4">
        <div v-for="(item, index) in diaryEntries" :key="index" class="bg-white p-4 rounded-lg shadow mb-3 flex items-center">
          <div class="w-12 h-12 rounded-full bg-green-100 flex items-center justify-center text-2xl mr-4">
    ...
    <div v-if="mode === 'add'" class="bg-gray-50 min-h-screen">
       <header class="bg-white p-4 flex justify-between items-center border-b sticky top-0 z-10">
        ...
        <button @click="saveEntry" class="bg-green-600 text-white px-6 py-2 rounded font-bold">Lưu</button>
      </header>
      ...
      <div class="p-4 space-y-4">
        <div class="bg-white p-4 rounded-lg shadow">
           <div class="mb-4">
            ...
            <input type="datetime-local" v-model="form.date" class="w-full p-3 border rounded bg-gray-50 text-lg">
          </div>
          <div>
        ...
        <div class="bg-white p-4 rounded-lg shadow">
          <label class="block text-sm font-medium text-gray-700 mb-3">Bạn vừa làm công việc gì?</label>
          <div class="grid grid-cols-3 gap-3">
            <button
              v-for="(act, idx) in activityTypes"
              :key="idx"
        ...
        <div v-if="form.type" class="bg-white p-4 rounded-lg shadow animate-fade-in">
          ...
          <div v-if="form.type === 'fertilizer'" class="space-y-3">
             <input type="text" placeholder="Tên loại phân (VD: NPK 20-20-15)" class="w-full p-3 border rounded">
             <div class="flex items-center">
                <input type="number" placeholder="Số lượng" class="flex-grow p-3 border rounded mr-2">
           ...
           <div v-if="form.type === 'spray'" class="space-y-3">
             ...
             <input type="text" placeholder="Phòng trừ đối tượng nào? (VD: Rầy nâu)" class="w-full p-3 border rounded">
          </div>

          <div v-if="!['fertilizer', 'spray'].includes(form.type)">
             <textarea placeholder="Ghi chú thêm về công việc..." rows="3" class="w-full p-3 border rounded"></textarea>
          </div>
        </div>

        <div class="bg-white p-4 rounded-lg shadow">
            <label class="block text-sm font-medium text-gray-700 mb-3">Hình ảnh (nếu có)</label>
            <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 flex flex-col items-center justify-center text-gray-400 bg-gray-50">
