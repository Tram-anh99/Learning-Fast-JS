...
def import_chu_so_huu(conn):
    ...
    query = """
        ...
        (ma_so_thue, ten_to_chuc, dia_chi, dien_thoai, email, loai_to_chuc)
        VALUES %s
        ON CONFLICT (ma_so_thue) DO NOTHING
    """
...
def import_vung_trong(conn):
    ...
    for _, row in df.iterrows():
        # Get chu_so_huu_id from ma_so_thue
        ma_so_thue = row.get('MaSoThue')
        if ma_so_thue:
            cursor.execute(
                "SELECT id FROM nongsan.to_chuc_ca_nhan WHERE ma_so_thue = %s",
                (ma_so_thue,)
            )
