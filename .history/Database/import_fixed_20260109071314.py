        ...
        try:
            ...
            for sheet_name, sheet_df in df_dict.items():
                ...
                for idx, row in sheet_df.iterrows():
                    ...
                    ma_phan_bon = f"PB{phan_bon_counter:05d}"  # PB00001, PB00002...
                    phan_bon_counter += 1
                    ...
                    try:
                        ...
                        """, (
                            ...
                            1                 # loai_phan_bon_id: Default (Phân đạm)
                        ))
                        rows_imported += 1
                        total_imported += 1
                ...
                if rows_imported > 0:
                    print(f"         ✅ Import {rows_imported} phân bón từ sheet này")
...
def import_thuoc_bvtv(conn):
    ...
    for file_name in excel_files:
        ...
        try:
            ...
            for sheet_name, sheet_df in df.items():
                ...
                potential_ten_thuoc_cols = ['TenThuoc', 'Tên thuốc', 'Ten thuoc', 'Tên thương phẩm', 'Tên sản phẩm']
                potential_hoat_chat_cols = ['HoatChat', 'Hoạt chất', 'Hoat chat', 'Thành phần hoạt chất', 'Thanh phan hoat chat']
                potential_ham_luong_cols = ['HamLuong', 'Hàm lượng', 'Ham luong']
                ...
                for idx, row in sheet_df.iterrows():
                    ...
                    ma_thuoc = f"TBVTV{thuoc_bvtv_counter:05d}" # TBVTV00001, TBVTV00002...
                    ...
                    try:
                        ...
                        """, (
                            ...
                            'Dữ liệu giả định từ Excel' if ma_thuoc.startswith('TBVTV') and not pd.notna(row.get('MoTa', None)) else row.get('MoTa', None) # Mô tả
                        ))
                        total_imported += 1
                    except Exception as e:
                        conn.rollback() # Rollback nếu có lỗi ở một row
                        print(f"      ⚠️  Lỗi import row {idx+1} trong {file_name} (sheet {sheet_name}): {str(e)[:100]}...")
    ...
    cursor.close()
    print(f"✅ HOÀN TẤT import thuốc BVTV. Tổng cộng đã import {total_imported} thuốc.")
