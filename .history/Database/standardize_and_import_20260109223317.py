...
def import_location_data(conn):
    ...
    for (province,) in provinces:
        ...
        cur.execute(
            "SELECT id FROM nongsan.tinh WHERE ten_tinh = %s", (std_province,))
        if cur.fetchone():
            continue
    ...
    if skipped_communes > 0:
        print(
            f"   ⚠️  Bỏ qua {skipped_communes} xã (không tìm thấy huyện tương ứng)")
...
def import_co_so_data(conn):
    ...
    for row in cur.fetchall():
        ...
        # Check if already exists
        cur.execute(
            "SELECT id FROM nongsan.co_so WHERE ma_co_so = %s", (ma_co_so,))
        if cur.fetchone():
            continue
        ...
        if province:
            cur.execute(
                "SELECT id FROM nongsan.tinh WHERE ten_tinh = %s", (province,))
            result = cur.fetchone()
            if result:
                tinh_id = result[0]
