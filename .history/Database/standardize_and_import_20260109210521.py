...
def import_co_so_data(conn):
    ...
    cur.execute("""
        INSERT INTO nongsan.to_chuc_ca_nhan (ten_to_chuc, loai_hinh, ngay_tao)
        VALUES ('Chưa xác định', 'Khác', NOW())
        ON CONFLICT DO NOTHING
        RETURNING id
    """)
    result = cur.fetchone()
    default_to_chuc_id = result[0] if result else 3  # Fallback to existing id
    conn.commit()
