...
def check_loai_hoat_dong_duplicates(conn):
    ...
    try:
        # Find duplicates
        cur.execute("""
            SELECT ten_hoat_dong, mo_ta, COUNT(*) as count
            FROM nongsan.loai_hoat_dong
            GROUP BY ten_hoat_dong, mo_ta
            HAVING COUNT(*) > 1
            ORDER BY count DESC
        """)
        ...
        if duplicates:
            ...
            for ten, mo_ta, count in duplicates:
                ...
                cur.execute("""
                    ...
                    WHERE ten_hoat_dong = %s AND (mo_ta = %s OR (mo_ta IS NULL AND %s IS NULL))
                    ORDER BY id
                """, (ten, mo_ta, mo_ta))
                ids = [row[0] for row in cur.fetchall()]
                print(f"      IDs: {ids}")
            
            # Ask to keep first, delete rest
            print(f"\n❓ Delete duplicates? (keep first ID, delete rest)")
            # Auto-delete for now
            total_deleted = 0
            for ten, mo_ta, count in duplicates:
                cur.execute("""
                    ...
                    AND ten_hoat_dong = %s AND (mo_ta = %s OR (mo_ta IS NULL AND %s IS NULL))
