...
def import_vung_cay_trong(conn):
    ...
    for vung_id in vung_trong_ids:
        ...
        selected_crops = random.sample(
            loai_cay_list, min(num_crops, len(loai_cay_list)))
...
def create_useful_views(conn):
    ...
    cur.execute("""
        ...
        SELECT 
            ...
            vt.thoi_gian_bat_dau_thu_hoach,
            vt.thoi_gian_ket_thuc_thu_hoach,
            vt.ngay_tao,
            vt.ngay_cap_nhat
...
def verify_enhancements(conn):
    ...
    print(
        f"   Có latitude:             {has_lat:>6} ({100*has_lat/total if total > 0 else 0:.1f}%)")
    print(
        f"   Có longitude:            {has_lon:>6} ({100*has_lon/total if total > 0 else 0:.1f}%)")
    ...
    print(f"   Tổng số cơ sở:           {total:>6}")
    print(
        f"   Có tinh_id:              {has_tinh_id:>6} ({100*has_tinh_id/total if total > 0 else 0:.1f}%)")
