...
Props: ├─ item (Object) - Single activity ├─ getActivityIcon (Function) Hiển
thị: ├─ Cột ngày tháng (dateMonth, dateDay) ├─ Tiêu đề hoạt động ├─ Vị trí thửa
đất (field) └─ Chi tiết hoạt động (details) Dùng: KHÔNG DÙNG → Orphaned! ❌

<!-- DiaryActivityHistory.vue -->
Props: └─ activities (Array) Hiển thị: ├─ Vòng lặp activities ├─ Icon màu sắc
theo loại hoạt động ├─ Tiêu đề + Mô tả ├─ Thời gian tương đối (Hôm nay, Hôm qua)
└─ Thời gian chính xác (HH:MM) Dùng: ✅ DiaryPage import và sử dụng
```
