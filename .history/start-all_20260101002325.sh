...
# Kiểm tra PostgreSQL port 5433 có đang chạy không
if lsof -Pi :5433 -sTCP:LISTEN -t >/dev/null 2>&1; then
    echo "✅ PostgreSQL running on port 5433"
else
    echo "⚠️  PostgreSQL không chạy trên port 5433"
    echo "   Please start PostgreSQL (DataGrip or other tool)"
    echo "   hoặc check port 5432:"
    
    if lsof -Pi :5432 -sTCP:LISTEN -t >/dev/null 2>&1; then
        echo "   ✅ PostgreSQL đang chạy trên port 5432"
        echo "   Update Backend/.env: DB_PORT=5432"
    else
        echo "   ❌ PostgreSQL không chạy!"
        echo "   Start PostgreSQL và chạy lại script này"
        exit 1
    fi
fi
