...
|------|-------|-----|---------|
| **Map Components** | 3 | 428 | Active usage |
| **Diary Components** | 6 | 878 | ⚠️ Some overlap |
...
| **TOTAL** | 25 | **2,901** | ✅ Build: 62 modules |
...
|-----------|--------|--------|
| DiaryActivityCard.vue | 🚫 UNUSED | Orphaned, not imported anywhere |
| DiaryHeader.vue | ⚠️ SIMPLE | Just title + date, maybe use slot |
| DiaryNavigation.vue | ⚠️ UNUSED | Not imported in DiaryPage |
| IconXxx.vue (5 files) | ✅ OKAY | Utility components, fine as is |
...
```
Frontend Total: 2,901 LOC (components)

By Size:
┌─────────────────────────────────────┐
│ DiaryActivityForm      474 lines 16% │
│ QRScanner             218 lines  7%  │
│ MapLayerSelector      203 lines  7%  │
│ SidebarHeader         184 lines  6%  │
│ DiaryActivitySelector 176 lines  6%  │
│ HomeDetailView        168 lines  6%  │
│ ChartsComponent       159 lines  5%  │
│ Others (17 files)     919 lines 32%  │
└─────────────────────────────────────┘

By Category:
┌──────────────────────────────────┐
│ Diary (6 files)      878 lines 30% │
│ Map (3 files)        428 lines 15%  │
│ Modal (2 files)      226 lines  8%  │
│ Filter (3 files)     396 lines 14%  │
│ Other (25 files)     973 lines 33%  │
└──────────────────────────────────┘
```
...
```
24 Components (2,901 LOC) + 4 Composables (758 LOC) = 3,659 LOC
```

### Proposed (Better Balance)
```
18 Components (2,200 LOC) + 10 Composables (1,500 LOC) = 3,700 LOC
         ↓                              ↑
    (Remove orphans,          (Extract logic)
     consolidate)
```
...
```
Current: ✅ 62 modules
After cleanup: 62 modules (no change to bundle size)
After refactor: ~58-60 modules (more logical grouping)
```
