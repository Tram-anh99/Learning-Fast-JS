...
from pydantic import BaseModel, Field, ConfigDict  # BaseModel: Base class, Field: Field config, ConfigDict: Model config
from typing import Optional, List  # Optional: Field có thể None, List: Array type
from datetime import date, datetime  # date: Date only (YYYY-MM-DD), datetime: Date + time
...
class VungTrongResponse(VungTrongBase):
    ...
    model_config = ConfigDict(from_attributes=True)  # Allow parse from SQLAlchemy model
...
class ChartDataset(BaseModel):
    ...
    backgroundColor: Optional[str | List[str]] = None # Màu nền (single hoặc array)
