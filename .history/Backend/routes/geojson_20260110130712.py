"""
========== ROUTES: GEOJSON API (Map Visualization) ==========

Mục đích:
API endpoints cho hiển thị bản đồ với GeoJSON format

Endpoints:
1. GET /api/geojson/provinces         - Polygon tỉnh/thành phố
2. GET /api/geojson/districts         - Polygon quận/huyện
3. GET /api/geojson/communes          - Polygon phường/xã
4. GET /api/geojson/farms/boundaries  - Polygon vùng trồng
5. GET /api/geojson/routes/lines      - LineString kết nối tỉnh
6. GET /api/geojson/info/{layer}/{id} - Popup info khi click
...
Frontend Components:
- Frontend/src/components/MapComponent.vue (Leaflet map)
- Frontend/src/components/MapLayerControl.vue (Layer control panel)
- Frontend/src/components/MapLayerSelector.vue (Layer selector)
- Frontend/src/composables/useMapLogic.js (GeoJSON loading logic)

Database Tables:
- nongsan.tinh (Provinces with x,y coordinates)
- nongsan.huyen (Districts with x,y coordinates)
- nongsan.xa (Communes with x,y coordinates)
- nongsan.vung_trong (Farms with coordinates)
- nongsan.toa_do_vung (Farm polygon coordinates)

Note:
- Coordinates stored as x (longitude), y (latitude)
- Creates bounding boxes if real polygons not available
- Returns empty FeatureCollection if no data
- Uses psycopg2 direct connection for performance

Author: Development Team
Date: January 10, 2026 (GeoJSON API implementation)
"""
...
async def get_farm_boundaries_geojson(
    ...
    try:
        ...
        for row in rows:
            ...
            center_x = float(tinh_x) + (vid % 10 - 5) * \
                0.05  # offset from province center
...
async def get_routes_as_lines():
    ...
    try:
        ...
        for idx, row in enumerate(rows):
            from_id, from_tinh, from_x, from_y, to_id, to_tinh, to_x, to_y = row
...
async def get_feature_info(layer: str, feature_id: int):
    ...
    try:
        ...
        if layer == "provinces":
            ...
            raise HTTPException(
                status_code=400, detail=f"Unknown layer: {layer}")

        row = cur.fetchone()
        if not row:
            raise HTTPException(
                status_code=404, detail=f"Feature not found: {layer}/{feature_id}")
