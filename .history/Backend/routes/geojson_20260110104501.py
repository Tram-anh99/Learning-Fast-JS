...
async def get_routes_as_lines():
    ...
    Example: Connect farms to markets
    """
    try:
        conn = get_db()
        cur = conn.cursor()
        
        # Get farm-market connections from vung_trong_thi_truong
        query = """
            SELECT 
                vttt.id,
                vt.id as vung_id,
                vt.ten_vung,
                tt.id as thi_truong_id,
                tt.ten_thi_truong,
                vt.tinh_id,
                t1.x as vung_x,
                t1.y as vung_y,
                tt.tinh_id as thi_truong_tinh_id,
                t2.x as thi_truong_x,
                t2.y as thi_truong_y
            FROM nongsan.vung_trong_thi_truong vttt
            JOIN nongsan.vung_trong vt ON vttt.vung_trong_id = vt.id
            JOIN nongsan.thi_truong tt ON vttt.thi_truong_id = tt.id
            LEFT JOIN nongsan.tinh t1 ON vt.tinh_id = t1.id
            LEFT JOIN nongsan.tinh t2 ON tt.tinh_id = t2.id
            WHERE t1.x IS NOT NULL AND t1.y IS NOT NULL
            AND t2.x IS NOT NULL AND t2.y IS NOT NULL
            LIMIT 100
        """
        ...
        features = []
        for row in rows:
            rid, vung_id, ten_vung, tt_id, ten_tt, tinh_id, vung_x, vung_y, tt_tinh_id, tt_x, tt_y = row
            
            # Create LineString from farm to market
            line_coords = [
                [float(vung_x), float(vung_y)],
                [float(tt_x), float(tt_y)]
            ]
            ...
            features.append(GeoJSONFeature(
                ...
                properties={
                    ...
                    "ten_vung": ten_vung,
                    "thi_truong_id": tt_id,
                    "ten_thi_truong": ten_tt,
                    "from_tinh_id": tinh_id,
                    "to_tinh_id": tt_tinh_id,
                    "layer": "routes"
                }
            ))
