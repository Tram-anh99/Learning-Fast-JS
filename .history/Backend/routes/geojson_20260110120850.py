...
async def get_routes_as_lines():
    ...
    try:
        ...
        # Create lines between provinces that have multiple farms
        query = """
            WITH province_pairs AS (
                SELECT DISTINCT
                    t1.id as from_tinh_id,
                    t1.ten_tinh as from_tinh,
                    t1.x as from_x,
                    t1.y as from_y,
                    t2.id as to_tinh_id,
                    t2.ten_tinh as to_tinh,
                    t2.x as to_x,
                    t2.y as to_y,
                    COUNT(vt.id) as connection_count
                FROM nongsan.vung_trong vt
                JOIN nongsan.tinh t1 ON vt.tinh_id = t1.id
                CROSS JOIN nongsan.tinh t2
                WHERE t1.id < t2.id
                AND t1.x IS NOT NULL AND t1.y IS NOT NULL
                AND t2.x IS NOT NULL AND t2.y IS NOT NULL
                GROUP BY t1.id, t1.ten_tinh, t1.x, t1.y, t2.id, t2.ten_tinh, t2.x, t2.y
                HAVING COUNT(vt.id) > 0
                LIMIT 50
            )
            SELECT * FROM province_pairs
        """
