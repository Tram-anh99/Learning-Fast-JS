...
class VungTrong(Base):
    ...
    # FK → chung_nhan.id (chứng nhận: VietGAP, GlobalGAP, etc.) - Comment nếu chưa có model
    
    # ========== HÀNH CHÍNH (Optional - Comment nếu chưa có models) ==========
...
class ToaDoVung(Base):
    ...
    vi_do = Column(Numeric, nullable=False)
    # Vĩ độ (latitude)
    # Example: 10.762622 (TP.HCM)
    
    kinh_do = Column(Numeric, nullable=False)
    # Kinh độ (longitude)
