...
import { mockDataVung } from "./statusHelpers";

// ========== STATE & REFS ==========
// Mảng dữ liệu gốc chứa tất cả vùng trồng (fetch từ API)
...
export const fetchFarmsData = async () => {
     ...
     try {
          ...
          danhSachGoc.value = farms.map((farm) => ({
               ...
               ma: farm.ma_vung,
               ten: farm.ten_vung,
               dienTich: farm.dien_tich ? `${farm.dien_tich}ha` : "0ha",
          ...
          danhSachGoc.value = mockDataVung.map((vung) => ({
               ...vung,
