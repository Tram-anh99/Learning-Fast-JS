 ...
 *   - Tích hợp dữ liệu thực từ useCropData.js
 *   - Hàm API integration để lấy data từ server
 */

import { ref, computed } from "vue";
import {
     getCropProductivityData,
     getExportMarketData,
     getAllCrops,
     getAllMarkets,
} from "./useCropData";
 ...
 * TỰ ĐỘNG TÍNH TOÁN từ dữ liệu chi tiết loại cây (useCropData.js)
 *
 * Structure:
 *   - market: Tên thị trường (Trung Quốc, Hoa Kỳ, Châu Âu, ...)
 *   - value: Giá trị xuất khẩu (USD) tính từ diện tích * năng suất * giá
 *   - cropCount: Số loại cây xuất khẩu vào thị trường này
 */
const exportData = ref(getExportMarketData());
 ...
 *
 * Structure:
 *   - tenCay: Tên loại cây (Lúa, Xoài, Thanh Long, ...)
 *   - nangSuat: Năng suất trung bình (tạ/ha)
 *   - dienTich: Tổng diện tích trồng loại cây này (ha)
 */
const cropData = ref(getCropProductivityData());
