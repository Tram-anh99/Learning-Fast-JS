 ...
 */
export const switchLayer = (layerName) => {
     if (!map.value || !tileLayers.value) return;

     // Xóa các layer cũ
     Object.keys(tileLayers.value).forEach(key => {
          try {
               map.value.removeLayer(tileLayers.value[key]);
          } catch (e) {
               // Ignore if layer not added yet
          }
     });

     // Thêm layer mới
     if (tileLayers.value[layerName]) {
          tileLayers.value[layerName].addTo(map.value);
