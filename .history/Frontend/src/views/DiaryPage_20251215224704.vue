...
<template>
  <div class="font-sans antialiased h-screen flex flex-col" style="background-color: #fbfced;">
    ...
    <main class="flex-1 w-full px-4 py-6 pb-[80px] mx-auto sm:px-6 lg:px-8 max-w-7xl overflow-y-auto scrollbar-custom">
      ...
      <section class="mb-8 bg-white rounded-2xl shadow-sm p-6" style="border: 1px solid #24504b;">
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          ...
          <button v-for="field in fields" :key="field.id" @click="selectField(field.id)" :class="[
            ...
            <div class="flex items-center justify-between mb-2">
              ...
              <span class="px-2 py-1 text-xs font-bold rounded-lg" style="background-color: rgba(36, 80, 75, 0.2); color: #24504b;">
                {{ field.ma }}
              </span>
            </div>
            <!-- Thông tin thửa đất -->
            <div class="space-y-1 text-sm">
              ...
              <p class="text-xs font-bold" :style="{ color: selectedField === field.id ? '#24504b' : 'rgba(36, 80, 75, 0.5)' }">
                {{ field.status }}
              </p>
            </div>
          </button>
