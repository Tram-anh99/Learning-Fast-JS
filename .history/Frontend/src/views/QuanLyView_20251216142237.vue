...
/**
 * showTableModal: Hiển thị bảng dữ liệu dạng modal trên mobile
 * Type: Ref<Boolean>
 */
const showTableModal = ref(false);

/**
 * showCropDetailsModal: Hiển thị chi tiết cây trồng dạng modal trên mobile
 * Type: Ref<Boolean>
 */
const showCropDetailsModal = ref(false);

/**
 * showChartsModal: Hiển thị biểu đồ dạng modal trên mobile
 * Type: Ref<Boolean>
 */
...
const toggleTableModal = () => {
      showTableModal.value = !showTableModal.value;
};
...
/**
 * Toggle crop details modal
 */
const toggleCropDetailsModal = () => {
      showCropDetailsModal.value = !showCropDetailsModal.value;
};
...
const closeCropDetailsModal = () => {
      showCropDetailsModal.value = false;
};
...
const toggleChartsModal = () => {
      showChartsModal.value = !showChartsModal.value;
};
...
<template>
    ...
    -->
            ...
            <!-- Button mở charts trên mobile -->
            <div class="block md:hidden">
                  <button @click="toggleChartsModal"
                        class="w-full px-4 py-3 bg-teal-700 text-white font-semibold rounded-lg shadow-md hover:bg-teal-800 transition-colors">
                        <i class="fas fa-chart-bar mr-2"></i>
                        Xem biểu đồ thống kê
                  </button>
            </div>

            <!-- Khu vực: Biểu đồ cột (trái) & Biểu đồ đường (phải) - Desktop view -->
            <!-- Responsive: Dọc trên mobile/tablet, ngang trên desktop -->
            <div class="hidden md:flex flex-col md:flex-row gap-5">
            ...
            <!-- Button mở crop details trên mobile -->
            <div class="block md:hidden">
                  <button @click="toggleCropDetailsModal"
                        class="w-full px-4 py-3 bg-teal-700 text-white font-semibold rounded-lg shadow-md hover:bg-teal-800 transition-colors">
            ...
            <!-- Chi tiết loại cây của vùng được chọn - Desktop view -->
            ...
            <!-- Button mở table trên mobile/tablet -->
            <div class="block md:hidden">
                  <button @click="toggleTableModal"
                        class="w-full px-4 py-3 bg-teal-700 text-white font-semibold rounded-lg shadow-md hover:bg-teal-800 transition-colors">
            ...
            <!-- Bảng danh sách vùng - Desktop view -->
    ...
    -->
            ...
            <!-- ========== TABLE MODAL - Mobile/Tablet ========== -->
            <!-- Modal overlay với backdrop mờ -->
            <Transition name="modal">
                  <div v-if="showTableModal"
                        class="fixed inset-0 z-50 flex items-start justify-center bg-black bg-opacity-50 md:hidden"
                        @click.self="closeTableModal">
                        <!-- Modal content -->
                        <div class="bg-white w-full max-w-4xl max-h-[90vh] mt-4 mx-4 rounded-t-2xl shadow-2xl overflow-hidden flex flex-col">
                              <!-- Modal header -->
                              <div class="flex items-center justify-between p-4 border-b bg-teal-700 text-white">
                                    <h3 class="text-lg font-bold">
                                          <i class="fas fa-table mr-2"></i>
                                          Danh sách Vùng trồng
                                    </h3>
                                    <button @click="closeTableModal"
                                          class="text-white hover:bg-teal-800 rounded-full w-8 h-8 flex items-center justify-center transition-colors">
            ...
            <!-- ========== CHARTS MODAL - Mobile/Tablet ========== -->
            <!-- Modal overlay với backdrop mờ -->
            <Transition name="modal">
                  <div v-if="showChartsModal"
                        class="fixed inset-0 z-50 flex items-start justify-center bg-black bg-opacity-50 md:hidden"
                        @click.self="closeChartsModal">
                        <!-- Modal content -->
                        <div class="bg-white w-full max-w-4xl max-h-[90vh] mt-4 mx-4 rounded-t-2xl shadow-2xl overflow-hidden flex flex-col">
                              <!-- Modal header -->
                              <div class="flex items-center justify-between p-4 border-b bg-teal-700 text-white">
                              ...
                              <div class="flex-1 overflow-auto p-4">
                                    <div class="flex flex-col gap-5">
                                          <!-- Bar Chart -->
                                          <div class="w-full p-4 shadow-md rounded-xl h-[450px]" style="background-color: white;">
                                                <BarChartComponent />
                                          </div>
                                          <!-- Line Chart -->
                                          <div class="w-full p-4 shadow-md rounded-xl h-[450px]" style="background-color: white;">
            ...
            <!-- ========== CROP DETAILS MODAL - Mobile/Tablet ========== -->
            <!-- Modal overlay với backdrop mờ -->
            <Transition name="modal">
                  <div v-if="showCropDetailsModal"
                        class="fixed inset-0 z-50 flex items-start justify-center bg-black bg-opacity-50 md:hidden"
                        @click.self="closeCropDetailsModal">
                        <!-- Modal content -->
                        <div class="bg-white w-full max-w-4xl max-h-[90vh] mt-4 mx-4 rounded-t-2xl shadow-2xl overflow-hidden flex flex-col">
                              <!-- Modal header -->
                              <div class="flex items-center justify-between p-4 border-b bg-teal-700 text-white">
                                    ...
                                    <button @click="closeCropDetailsModal"
                                          class="text-white hover:bg-teal-800 rounded-full w-8 h-8 flex items-center justify-center transition-colors">
                                          <i class="fas fa-times text-xl"></i>
                                    </button>
                              </div>
