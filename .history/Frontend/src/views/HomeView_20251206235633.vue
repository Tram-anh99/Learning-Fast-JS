...
.floating-sidebar {
  position: absolute;
  top: 20px; left: 20px; bottom: 20px; /* Cách đều trên dưới trái */
  width: 340px;
  
  /* Hiệu ứng kính */
  background: rgba(255, 255, 255, 0.85); /* Trắng đục 85% để dễ đọc chữ */
...
.sidebar-header {
  padding: 16px 20px;
  background-color: var(--primary-color); /* Dùng màu xanh của App.vue */
...
.sidebar-tabs {
  display: flex; border-bottom: 1px solid rgba(0,0,0,0.1); background: rgba(255,255,255,0.5);
}
.tab-btn {
...
.tab-btn.active { color: #1b4332; border-bottom-color: #1b4332; font-weight: bold; background: rgba(255,255,255,0.8); }
...
.layer-item:hover { transform: translateY(-2px); box-shadow: 0 5px 15px rgba(0,0,0,0.1); border-color: #4caf50; }

.item-icon { width: 32px; height: 32px; background: #f1f8e9; color: #2e7d32; border-radius: 50%; display: flex; align-items: center; justify-content: center; }
...
.tool-btn {
  ...
  cursor: pointer; color: #444; font-size: 1rem;
  transition: all 0.2s;
}
