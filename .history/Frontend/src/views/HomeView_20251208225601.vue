...
export default {
  // ... các phần data, setup cũ ...

  methods: {
    // Hàm này sẽ chạy khi bấm nút
    goToTraceability(maSanPham) {
      console.log("Đang chuyển sang trang truy xuất cho mã:", maSanPham);
      
      // Chuyển hướng sang trang TraceabilityPage.vue
      // 'Traceability' là tên (name) bạn đã đặt trong router/index.js
      this.$router.push({ 
        name: 'Traceability', 
        params: { id: maSanPham } 
      });
    ...
    // ... các methods khác của bạn (nếu có) ...
  }
}
