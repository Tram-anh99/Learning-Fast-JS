...
// 2. Dữ liệu mẫu (Database ảo)
// (Đây là danh sách gốc, không bao giờ thay đổi)
const rawList = [
  { id: 1, name: 'Xoài Cát Hòa Lộc', status: 'Đang canh tác', type: 'canhtac', img: 'https://bit.ly/3R9u...'},
  { id: 2, name: 'Thanh Long Ruột Đỏ', status: 'Cảnh báo dịch hại', type: 'canhtac', img: 'https://bit.ly/3...'},
  { id: 3, name: 'Lúa ST25', status: 'Đang thu hoạch', type: 'thuhoach', img: 'https://bit.ly/3...'},
  { id: 4, name: 'Sầu Riêng Ri6', status: 'Đang canh tác', type: 'canhtac', img: 'https://bit.ly/3...'},
]

// 3. Danh sách Đã Lọc (Dùng cái này để hiển thị ra màn hình)
const filteredList = computed(() => {
  // Nếu không nhập gì thì hiện hết
  if (!searchQuery.value) return rawList
  
  // Có nhập thì lọc theo tên (không phân biệt hoa thường)
...
<template>
  ...
  <div class="webgis-container">
    ...
    <aside class="floating-sidebar">
      ...
      <div class="sidebar-header" :class="{ 'detail-mode': vungDangXem }">
        ...
        <div class="relative">
          ...
          <input 
            ...
            class="block w-full py-2 pl-10 pr-3 leading-5 text-green-100 placeholder-green-400 transition duration-150 ease-in-out bg-green-800 border-transparent rounded-lg focus:outline-none focus:bg-green-700 focus:ring-1 focus:ring-green-400 focus:border-green-400 sm:text-sm"
          >
          
          <div class="absolute inset-y-0 right-0 flex items-center pr-2">
            <span class="bg-green-700 text-green-100 text-xs px-2 py-0.5 rounded-full border border-green-600">
      ...
      <div class="bg-white p-2 h-[calc(100vh-200px)] overflow-y-auto">
        
        <div 
          v-for="item in filteredList" 
          :key="item.id" 
          class="flex gap-3 p-3 mb-2 transition bg-white border border-gray-200 shadow-sm cursor-pointer rounded-xl hover:shadow-md"
        >
            <img :src="item.img" class="object-cover w-16 h-16 bg-gray-100 rounded-lg">
            
            <div>
              <h4 class="font-bold text-gray-800">{{ item.name }}</h4>
              
              <span 
                class="text-[10px] px-2 py-1 rounded-full font-bold mt-1 inline-block"
                :class="{
                  'bg-green-100 text-green-700': item.status === 'Đang canh tác',
                  'bg-yellow-100 text-yellow-700': item.status === 'Đang thu hoạch',
                  'bg-red-100 text-red-700': item.status.includes('dịch hại')
                }"
              >
                {{ item.status }}
              </span>
        ...
        <div v-if="filteredList.length === 0" class="py-10 text-center text-gray-400">
            <p>Không tìm thấy nông sản nào.</p>
        </div>
