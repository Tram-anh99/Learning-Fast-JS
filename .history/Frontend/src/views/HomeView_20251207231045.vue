    ...
    <aside class="floating-sidebar">
      ...
      <div v-if="!vungDangXem" class="list-view">
        <div class="sidebar-tabs">
          <button class="tab-btn" :class="{ active: boLocHienTai === 'all' }" @click="boLocHienTai = 'all'">Tất cả</button>
          <button class="tab-btn" :class="{ active: boLocHienTai === 'canh_tac' }" @click="boLocHienTai = 'canh_tac'">Canh tác</button>
          <button class="tab-btn" :class="{ active: boLocHienTai === 'thu_hoach' }" @click="boLocHienTai = 'thu_hoach'">Thu hoạch</button>
        </div>
        ...
        <div class="sidebar-content">
          <ul class="layer-list">
            <li v-for="vung in danhSachHienThi" :key="vung.id" class="layer-item" @click="chonVung(vung)">
              ...
              <div class="item-status" :class="getClassTrangThai(vung.trangThai)">{{ getTextTrangThai(vung.trangThai) }}</div>
            </li>
          </ul>
        </div>
      </div>
...
<style scoped>
/* --- GIỮ CSS LAYOUT CŨ --- */
.webgis-container { position: absolute; top: 0; left: 0; width: 100%; height: 100%; display: flex; }
.map-container { width: 100%; height: 100%; z-index: 0; background-color: #e5e7eb; }

/* --- SIDEBAR CHO KHÁCH --- */
.floating-sidebar {
  position: absolute; top: 10px; left: 10px; bottom: 10px; width: 360px; /* Rộng hơn chút để hiện ảnh đẹp */
  ...
  display: flex; flex-direction: column; z-index: 1000; border: 1px solid rgba(255,255,255,0.5); overflow: hidden;
}

/* HEADER */
.sidebar-header {
  padding: 16px 20px; background: linear-gradient(to right, #1b4332, #2d6a4f); color: white;
  display: flex; align-items: center; transition: all 0.3s;
}
.sidebar-header h3 { margin: 0; font-size: 1.1rem; font-weight: 600; }
.header-content { width: 100%; display: flex; justify-content: space-between; align-items: center; }
.header-content.detail { justify-content: flex-start; gap: 15px; }
.count-badge { background: rgba(255,255,255,0.2); padding: 2px 8px; border-radius: 10px; font-size: 0.8rem; }
.btn-back { background: none; border: none; color: white; font-size: 1.2rem; cursor: pointer; }

/* LIST VIEW */
.sidebar-tabs { display: flex; border-bottom: 1px solid rgba(0,0,0,0.1); background: rgba(255,255,255,0.3); padding: 5px; gap: 5px; }
.tab-btn { flex: 1; padding: 8px; border: none; background: none; cursor: pointer; font-size: 0.85rem; font-weight: 500; color: #333; border-radius: 6px; }
.tab-btn.active { background: white; color: #1b4332; font-weight: bold; box-shadow: 0 2px 5px rgba(0,0,0,0.1); }
.list-view, .detail-view { flex-grow: 1; overflow-y: auto; display: flex; flex-direction: column; }
.sidebar-content { padding: 10px; }
.layer-list { list-style: none; padding: 0; margin: 0; }
.layer-item {
  background: rgba(255,255,255,0.6); border-radius: 12px; padding: 10px; margin-bottom: 10px;
  display: flex; align-items: center; gap: 12px; cursor: pointer; transition: all 0.2s; border: 1px solid white;
}
.layer-item:hover { transform: translateY(-3px); background: white; box-shadow: 0 5px 15px rgba(0,0,0,0.1); }
.item-thumb { width: 50px; height: 50px; border-radius: 8px; background-size: cover; background-position: center; flex-shrink: 0; }
.item-info { flex-grow: 1; display: flex; flex-direction: column; }
.item-name { font-weight: 700; font-size: 0.95rem; color: #1b4332; }
.item-sub { font-size: 0.75rem; color: #555; display: flex; align-items: center; gap: 4px; }
.item-status { font-size: 0.65rem; padding: 2px 6px; border-radius: 4px; color: white; white-space: nowrap; }
.status-green { background: #4caf50; } .status-red { background: #ef5350; } .status-yellow { background: #ffca28; color: #333; }
...
.detail-cover .overlay {
  width: 100%; padding: 15px; background: linear-gradient(to top, rgba(0,0,0,0.8), transparent); color: white;
}
.detail-cover h2 { margin: 0; font-size: 1.4rem; text-shadow: 0 2px 4px rgba(0,0,0,0.5); }
.cert-badge { background: #ffca28; color: #333; padding: 2px 8px; border-radius: 4px; font-size: 0.75rem; font-weight: bold; margin-top: 5px; display: inline-block; }

.detail-content { padding: 15px; flex-grow: 1; display: flex; flex-direction: column; gap: 20px; }
.info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
.info-box { background: rgba(255,255,255,0.5); padding: 10px; border-radius: 8px; border: 1px solid white; }
.info-box label { display: block; font-size: 0.75rem; color: #666; margin-bottom: 2px; }
.info-box strong { color: #1b4332; font-size: 1rem; }

/* TIMELINE (DÒNG THỜI GIAN) */
.timeline-section h4 { margin: 0 0 15px 0; color: #1b4332; border-bottom: 2px solid rgba(27, 67, 50, 0.1); padding-bottom: 5px; }
.timeline { border-left: 2px solid #ddd; margin-left: 5px; padding-left: 20px; }
.timeline-item { position: relative; margin-bottom: 20px; }
.time-dot { width: 12px; height: 12px; background: #4caf50; border-radius: 50%; position: absolute; left: -27px; top: 5px; border: 2px solid white; box-shadow: 0 0 0 2px #4caf50; }
.time-date { font-size: 0.75rem; color: #888; display: block; margin-bottom: 2px; }
.time-action { color: #333; display: block; font-weight: 600; }
.time-desc { margin: 2px 0 0 0; font-size: 0.85rem; color: #555; }

.btn-qr {
  margin-top: auto; background: #1b4332; color: white; border: none; padding: 12px; border-radius: 8px; cursor: pointer; font-weight: bold; display: flex; align-items: center; justify-content: center; gap: 8px;
}
.btn-qr:hover { background: #2d6a4f; }
</style>