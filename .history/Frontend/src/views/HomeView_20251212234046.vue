...
<template>
  <div class="webgis-container">
    ...
    <!-- Layer Selector Control - Click to show -->
    <div class="absolute z-20 top-6 right-6">
      <!-- Button -->
      <button @click="toggleLayerSelector" class="p-2.5 bg-white rounded-lg shadow-lg hover:shadow-xl transition-shadow">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-gray-700" viewBox="0 0 20 20" fill="currentColor">
          <path d="M10.5 1.5H3.75A2.25 2.25 0 001.5 3.75v12.5A2.25 2.25 0 003.75 18.5h12.5a2.25 2.25 0 002.25-2.25V9.5M10 5.5L3 9v8M17 2v6m0 0l-2-2m2 2l2-2" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" fill="none"/>
        </svg>
      </button>

      <!-- Dropdown menu - Click to toggle -->
      <div v-if="showLayerSelector" class="absolute top-12 right-0 bg-white rounded-lg shadow-lg p-3 w-48">
        <div class="text-xs font-bold text-gray-700 mb-2">Lớp bản đồ</div>
        <div class="space-y-2">
          <label class="flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-2 rounded">
            <input 
              ...
              @change="switchLayer('satellite'); closeLayerSelector()"
              class="w-4 h-4"
            >
            <span class="text-sm">Ảnh vệ tinh</span>
          </label>
          <label class="flex items-center gap-2 cursor-pointer hover:bg-gray-50 p-2 rounded">
            <input 
              type="radio" 
              :value="'street'" 
              v-model="selectedLayer"
              @change="switchLayer('street'); closeLayerSelector()"
              class="w-4 h-4"
            >
            <span class="text-sm">Bản đồ đường phố</span>
          </label>
        </div>
      </div>
    </div>
