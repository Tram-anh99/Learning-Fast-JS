...
// Dữ liệu mẫu (Có tọa độ 3 vùng để vẽ)
const danhSachVung = ref([
  {
    id: 1, ma: 'VT-001', ten: 'Xoài Cát Hòa Lộc', dienTich: '5ha', trangThai: 'canh_tac',
    toaDo: [[10.762, 106.660], [10.770, 106.670], [10.760, 106.670]] // Hình tam giác 1
  },
  {
    id: 2, ma: 'VT-002', ten: 'Thanh Long Ruột Đỏ', dienTich: '3.2ha', trangThai: 'sau_benh',
    toaDo: [[10.780, 106.680], [10.790, 106.690], [10.780, 106.690]] // Hình tam giác 2
  },
  {
    id: 3, ma: 'VT-003', ten: 'Lúa ST25', dienTich: '10ha', trangThai: 'thu_hoach',
    toaDo: [[10.750, 106.650], [10.800, 106.650], [10.745, 106.650]] // Hình tam giác 3
  },
]);

// 1. Hàm lấy màu CSS cho Sidebar (Badge)
...
// 2. Hàm lấy MÀU SẮC CHO BẢN ĐỒ (Hex Color)
...
// Hàm zoom khi click vào danh sách
const zoomToVung = (vung) => {
  if (map.value && vung.toaDo) {
    // Tính tâm của Polygon để zoom vào giữa
    const polygon = L.polygon(vung.toaDo);
    const center = polygon.getBounds().getCenter();

    map.value.flyTo(center, 16, { duration: 1.5 });
...
onMounted(() => {
  ...
  // --- QUAN TRỌNG: VÒNG LẶP VẼ POLYGON ---
  // Duyệt qua danh sách vùng và vẽ từng cái lên bản đồ
  danhSachVung.value.forEach(vung => {
    if (vung.toaDo) {
      const mauSac = getMapColor(vung.trangThai); // Lấy màu dựa theo trạng thái

      L.polygon(vung.toaDo, {
        color: mauSac,       // Màu viền
        fillColor: mauSac,   // Màu nền
        fillOpacity: 0.5,    // Độ trong suốt (để nhìn thấy đất bên dưới)
        weight: 2            // Độ dày viền
      })
        .addTo(map.value)
        .bindPopup(`
        <div style="text-align: center;">
          <b style="color:${mauSac}">${vung.ten}</b><br>
          Diện tích: ${vung.dienTich}<br>
          Trạng thái: ${getTextTrangThai(vung.trangThai)}
        </div>
      `);
    }
  });
