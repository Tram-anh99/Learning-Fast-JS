...
// Đây là các điểm nóng để vẽ Heatmap
...
// Hàm vẽ bản đồ nhiệt (Heatmap Simulation bằng hình tròn)
const batCheDoSauBenh = () => {
  cheDoXem.value = 'sau_benh';
  lopSauBenh.value.clearLayers();
  
  // Vẽ các vòng tròn đỏ lan tỏa để mô phỏng dịch bệnh
  diemNongSauBenh.forEach(coord => {
    L.circle(coord, {
      color: 'red',
      fillColor: '#f03',
      fillOpacity: 0.4,
      radius: 300 // Bán kính lây lan 300m
...
const batCheDoHanhChinh = () => {
  ...
  lopSauBenh.value.clearLayers(); // Tắt lớp sâu bệnh
};
...
onMounted(() => {
  ...
  // 1. Khởi tạo Map (Giao diện Tối để làm nổi bật dữ liệu)
  map.value = L.map(mapContainer.value, { zoomControl: false }).setView([10.765, 106.660], 13);
  L.control.zoom({ position: 'bottomright' }).addTo(map.value);

  // Dùng nền bản đồ sáng sủa (CartoDB Positron) để dễ nhìn số liệu
  L.tileLayer('https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png', {
    attribution: '© CartoDB'
  }).addTo(map.value);

  // Thêm lớp quản lý sâu bệnh vào map
  lopSauBenh.value.addTo(map.value);

  // Vẽ các điểm vùng trồng (Marker)
          ...
          <tbody>
            <tr v-for="vung in danhSachVung" :key="vung.id">
              ...
              <td>
                ...
                <button class="btn-action warn" title="Gửi cảnh báo"><i class="fas fa-bell"></i></button>
              </td>
            </tr>
          </tbody>
        </table>
...
.stat-card {
  flex: 1; background: white; padding: 15px 20px; border-radius: 12px;
  box-shadow: 0 2px 5px rgba(0,0,0,0.05); display: flex; align-items: center; gap: 15px;
}
...
.map-controls button {
  ...
  text-align: left; cursor: pointer; font-size: 0.9rem; color: #333; transition: all 0.2s;
}
.map-controls button:hover { background: #e2e6ea; }
.map-controls button.active { background: #1b4332; color: white; border-color: #1b4332; }
.map-controls button i { width: 20px; text-align: center; margin-right: 5px; }

/* 3. ADMIN PANEL (TABLE) */
.admin-panel {
  flex: 1.5; background: white; border-radius: 12px; box-shadow: 0 2px 5px rgba(0,0,0,0.05);
  display: flex; flex-direction: column; overflow: hidden;
}
.panel-header {
  padding: 15px 20px; border-bottom: 1px solid #e2e8f0; display: flex; justify-content: space-between; align-items: center;
}
...
.btn-action.warn { background: #ffedd5; color: #ea580c; }
</style>