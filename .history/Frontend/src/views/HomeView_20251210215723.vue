...
const danhSachHienThi = computed(() => {
  ...
  // 3. Lọc theo từ khóa (Đã bọc kỹ để không bị lỗi)
  if (searchQuery.value) {
    const keyword = searchQuery.value.toLowerCase().trim();
    
    result = result.filter(v => {
      // Ép kiểu về chuỗi rỗng nếu không có dữ liệu (tránh lỗi undefined)
      const ten = (v.ten || v.name || v.tenNongSan || '').toString().toLowerCase();
      const ma = (v.ma || v.id || '').toString().toLowerCase();
