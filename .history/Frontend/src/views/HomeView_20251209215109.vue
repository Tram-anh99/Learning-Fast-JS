...
import { onMounted, shallowRef, watch, markRaw } from 'vue';
import L from 'leaflet';
import 'leaflet/dist/leaflet.css';
...
// --- QUAN TRỌNG: Dùng shallowRef thay vì ref ---
// shallowRef giúp Vue không can thiệp sâu vào object của Leaflet -> Máy mát hơn hẳn
const map = shallowRef(null);
const layerGroup = shallowRef(null);
...
const chonVung = (vung) => {
  if (map.value && vung.toaDo) {
    ...
    // flyTo là animation nặng, đảm bảo duration hợp lý
