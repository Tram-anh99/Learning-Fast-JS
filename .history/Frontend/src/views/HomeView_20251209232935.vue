...
const danhSachHienThi = computed(() => {
  ...
  let result = danhSachGoc;

  // 2. Lọc theo TRẠNG THÁI (Logic cũ của bạn)
  if (boLocHienTai.value !== 'all') {
    result = result.filter(v => v.trangThai === boLocHienTai.value);
  }

  // 3. Lọc theo TỪ KHÓA TÌM KIẾM (Logic mới thêm)
  if (searchQuery.value) {
    const keyword = searchQuery.value.toLowerCase().trim(); // Chuyển về chữ thường để tìm cho dễ
    
    result = result.filter(v => {
      // Tìm theo Tên nông sản (v.ten) HOẶC Mã số (v.ma)
      // Lưu ý: Hãy kiểm tra xem trong data của bạn là 'v.ten' hay 'v.name' nhé!
      return (v.ten && v.ten.toLowerCase().includes(keyword)) || 
