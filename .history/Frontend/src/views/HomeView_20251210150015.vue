...
danhSachGoc.valueconst danhSachGoc = [
  {
...
const danhSachHienThi = computed(() => {
  ...
  // 2. Lọc theo bộ lọc trạng thái (Logic cũ)
  if (boLocHienTai.value !== 'all') {
    result = result.filter(v => v.trangThai === boLocHienTai.value);
  }

  // 3. Lọc theo từ khóa tìm kiếm (Logic mới)
  if (searchQuery.value) {
    const keyword = searchQuery.value.toLowerCase().trim();
    
    result = result.filter(v => {
      // Sửa 'v.ten' thành tên trường đúng trong dữ liệu của bạn (VD: v.name, v.productName...)
      const ten = v.ten ? v.ten.toLowerCase() : '';
      const ma = v.ma ? v.ma.toLowerCase() : '';
