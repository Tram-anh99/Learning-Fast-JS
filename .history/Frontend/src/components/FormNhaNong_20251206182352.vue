      ...
      <div class="modal-body">
        ...
        <div class="log-input-box">
          <div class="log-row">
            ...
            <input v-model="logMoi.chiTiet" type="text" placeholder="Tên thuốc/phân, liều lượng...">
          ...
          <table>
            ...
            <tbody>
              <tr v-for="(log, index) in form.nhatKy" :key="index">
                <td>{{ log.ngay }}</td>
                <td>
                  <span class="badge" :class="log.hanhDong">{{ log.hanhDong }}</span>
                </td>
                <td>{{ log.chiTiet }}</td>
...
.modal-overlay {
  ...
  align-items: center;
  z-index: 2000;
  backdrop-filter: blur(3px); /* Hiệu ứng làm mờ nền map */
}
...
.section-title {
  font-weight: bold; margin-bottom: 10px; color: #2c3e50; border-bottom: 2px solid #eee; padding-bottom: 5px;
  margin-top: 10px;
}
...
.form-group label { font-size: 0.85rem; color: #666; margin-bottom: 5px; }
.form-group input, .form-group select {
  padding: 8px; border: 1px solid #ddd; border-radius: 6px; outline: none;
}
.form-group input:focus { border-color: #42b883; }

/* Log Styles */
.log-input-box { background: #f8f9fa; padding: 10px; border-radius: 8px; margin-bottom: 10px; }
.log-row { display: flex; gap: 5px; }
.log-row input[type="text"] { flex-grow: 1; }
.btn-add { background: #42b883; color: white; border: none; width: 30px; border-radius: 4px; cursor: pointer; }

table { width: 100%; border-collapse: collapse; font-size: 0.9rem; }
th { text-align: left; color: #888; font-weight: normal; padding: 5px; }
td { padding: 8px 5px; border-bottom: 1px solid #eee; }

.badge { padding: 2px 6px; border-radius: 4px; font-size: 0.75rem; color: white; background: #95a5a6; }
.badge.bon_phan { background: #e67e22; }
.badge.phun_thuoc { background: #e74c3c; }
.badge.thu_hoach { background: #f1c40f; color: #333; }

.delete-btn { cursor: pointer; color: #aaa; }
.delete-btn:hover { color: red; }

.modal-footer {
  padding: 15px 20px; background: #f8f9fa; display: flex; justify-content: flex-end; gap: 10px;
}
.btn-save { background: #42b883; color: white; padding: 8px 20px; border: none; border-radius: 6px; cursor: pointer; font-weight: bold; }
.btn-cancel { background: white; border: 1px solid #ddd; padding: 8px 15px; border-radius: 6px; cursor: pointer; }
