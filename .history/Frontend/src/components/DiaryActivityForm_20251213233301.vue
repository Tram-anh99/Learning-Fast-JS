  ...
  <div class="sticky top-24">
    ...
    <div class="bg-white rounded-3xl shadow-xl border border-[#2E7D32]/10 overflow-hidden relative">
      ...
      <div class="p-6 sm:p-8">
        <!-- Thông tin vùng trồng (mã số WebGIS) - Hiển thị nếu có selectedField -->
        <div v-if="selectedField" class="mb-6 p-4 bg-[#FFF3E0] border-2 border-[#FFB74D] rounded-xl">
          <div class="flex items-start justify-between gap-3">
            <div>
              <p class="text-xs font-bold text-[#E65100] uppercase tracking-wider mb-1">Vùng trồng (WebGIS)</p>
              <p class="text-sm font-bold text-gray-800">{{ selectedField.name }}</p>
              <p class="text-xs text-gray-600 mt-1">Diện tích: {{ selectedField.area }} ha • {{ selectedField.crop }}</p>
            </div>
            <div class="px-3 py-2 bg-white rounded-lg border-2 border-[#E65100] text-center">
              <p class="text-xs font-bold text-[#E65100] uppercase tracking-wider">Mã số</p>
              <p class="text-lg font-black text-[#E65100]">{{ selectedField.ma }}</p>
            </div>
          </div>
        </div>
