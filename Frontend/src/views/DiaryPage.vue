<template>
      <div class="min-h-screen bg-gray-50 text-gray-800 font-sans pb-24">
            ...
            <div v-if="viewMode === 'list'">
                  ...
                  <div class="px-4 py-3 bg-white shadow-sm mb-4">
                        <div class="flex space-x-2 overflow-x-auto no-scrollbar py-1">
                              <button
                                    class="px-4 py-1.5 bg-green-100 text-green-800 rounded-full text-sm font-semibold whitespace-nowrap border border-green-200">Tất
                                    cả</button>
                              <button
                                    class="px-4 py-1.5 bg-gray-100 text-gray-600 rounded-full text-sm font-medium whitespace-nowrap border border-transparent">Thửa
                                    A</button>
                              <button
                                    class="px-4 py-1.5 bg-gray-100 text-gray-600 rounded-full text-sm font-medium whitespace-nowrap border border-transparent">Thửa
                                    B</button>
                  ...
                  <div class="px-4 space-y-3">
                        <div v-for="(item, index) in diaryList" :key="index"
                              class="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex items-start active:bg-gray-50 transition duration-150">
                              <div
                                    :class="`flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center text-2xl mr-4 ${getIconBgColor(item.type)}`">
                                    {{ getActivityIcon(item.type) }}
                              </div>

                              <div class="flex-grow">
                                    <div class="flex justify-between items-start">
                                          <h3 class="font-bold text-gray-800 text-lg leading-tight">{{ item.title }}
                                          </h3>
            ...
            <div v-if="viewMode === 'create'" class="animate-fade-in-up">
                  <header
                        class="bg-white border-b border-gray-200 px-4 py-3 flex justify-between items-center sticky top-0 z-50 shadow-sm">
                        <button @click="viewMode = 'list'"
                              class="text-gray-500 hover:text-gray-700 font-medium py-2 px-2 -ml-2">
                              Hủy bỏ
                        </button>
                        <h1 class="text-lg font-bold text-gray-800">Ghi chép mới</h1>
                        <button @click="saveEntry"
                              class="bg-green-600 text-white px-5 py-2 rounded-lg font-bold shadow-sm hover:bg-green-700 active:bg-green-800 transition">
                              Lưu
                        </button>
                  </header>
                  ...
                  <div class="p-4 space-y-6">
                        ...
                        <section class="bg-white p-4 rounded-xl shadow-sm border border-gray-100 space-y-4">
                              <div>
                                    <label class="block text-sm font-semibold text-gray-700 mb-2">Thời gian thực
                                          hiện</label>
                                    <input type="datetime-local" v-model="form.timestamp"
                                          class="w-full p-3 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none transition text-gray-700 font-medium">
                              </div>
                              <div>
                                    <label class="block text-sm font-semibold text-gray-700 mb-2">Thửa ruộng</label>
                                    <div class="relative">
                                          <select v-model="form.field"
                                                class="w-full p-3 bg-gray-50 border border-gray-300 rounded-lg appearance-none focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none text-gray-700 font-medium">
                        ...
                        <section>
                              ...
                              <div class="grid grid-cols-3 gap-3">
                                    <button v-for="act in activityTypes" :key="act.id" @click="form.type = act.id"
                                          :class="[
                        ...
                        <section v-if="form.type"
                              class="bg-white p-4 rounded-xl shadow-sm border border-green-100 animate-fade-in ring-1 ring-green-500/20">
                              <h3
                                    class="text-green-700 font-bold mb-4 flex items-center border-b border-green-100 pb-2">
                              ...
                              <div v-if="form.type === 'fertilizer'" class="space-y-4">
                                    <div>
                                          ...
                                          <input v-model="form.detailName" type="text"
                                                placeholder="VD: NPK, Đạm Phú Mỹ..."
                                                class="mt-1 w-full p-3 border rounded-lg focus:ring-green-500 focus:border-green-500">
                                    </div>
                                    <div>
                                          <label class="text-xs font-bold text-gray-500 uppercase">Khối lượng</label>
                                          <div class="flex items-center mt-1">
                                                <input v-model="form.detailAmount" type="number" placeholder="0"
                                                      class="w-full p-3 border rounded-l-lg focus:ring-green-500 focus:border-green-500">
                                                <span
                                                      class="bg-gray-100 border border-l-0 border-gray-200 p-3 rounded-r-lg font-bold text-gray-600">Kg</span>
                                          </div>
                                    </div>
                              </div>
                              ...
                              <div v-else-if="form.type === 'spray'" class="space-y-4">
                                    <div>
                                          ...
                                          <input v-model="form.detailName" type="text" placeholder="VD: Chess 50WG..."
                                                class="mt-1 w-full p-3 border rounded-lg focus:ring-green-500 focus:border-green-500">
                                    </div>
                                    <div>
                                          <label class="text-xs font-bold text-gray-500 uppercase">Đối tượng phòng
                                                trừ</label>
                                          <input v-model="form.detailTarget" type="text"
                                                placeholder="VD: Rầy nâu, Đạo ôn..."
                                                class="mt-1 w-full p-3 border rounded-lg focus:ring-green-500 focus:border-green-500">
                                    </div>
                              </div>
                              ...
                              <div v-else class="space-y-4">
                                    <div>
                                          <label class="text-xs font-bold text-gray-500 uppercase">Ghi chú công
                                                việc</label>
                                          <textarea v-model="form.note" rows="3" placeholder="Mô tả chi tiết..."
                                                class="mt-1 w-full p-3 border rounded-lg focus:ring-green-500 focus:border-green-500"></textarea>
                                    </div>
                              </div>
                        </section>
                        ...
                        <section class="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                              ...
                              <label
                                    class="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 hover:border-green-400 transition">
                              ...
                              <div v-if="form.imagePreview" class="mt-3 relative">
                                    ...
                                    <button @click="form.imagePreview = null"
