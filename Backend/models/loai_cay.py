...
class LoaiCay(Base):
    ...
    nhom_cay_id = Column(Integer, ForeignKey("nongsan.nhom_cay.id"))
    # - FK → nhom_cay.id (nhóm cây: lương thực, công nghiệp, ...)
...
class VungCayTrong(Base):
    ...
    giong_id = Column(Integer, ForeignKey("nongsan.giong_bao_ho.id"))
    # - FK đến giong_bao_ho.id (giống bảo hộ - nếu có)
    
    # ========== CROP DETAILS ==========
    dien_tich = Column(Numeric)
    # - Diện tích trồng loại cây này trong vùng (hecta) - NOTE: Numeric không phải Float
    # - nullable=True: Có thể chưa có dữ liệu
    # Example: 2.5 (2.5 hecta)
    # Note: Sum(dien_tich) <= vung_trong.dien_tich
    
    nang_suat = Column(Numeric)
    # - Năng suất (tấn/vụ hoặc tấn/năm) - NOTE: DB có nang_suat không phải san_luong_du_kien
    # Example: 4.5 (4.5 tấn)
    ...
    gia_xuat_khau = Column(Numeric)
    # - Giá xuất khẩu (USD/kg hoặc USD/tấn)
