...
def guess_column_type(column_name, sample_values):
    ...
    # Lọc giá trị hợp lệ
    valid_samples = [str(v) for v in sample_values if pd.notna(
        v) and str(v).strip() != '']
    ...
    if 15 <= avg_length <= 100:
        ...
        if has_product_indicators:
            ...
            pesticide_words = ['sâu', 'sau', 'nấm', 'nam', 'cỏ', 'co',
                               'rệp', 'rep', 'bọ', 'bo', 'lúa', 'lua', 'cải', 'cai']
            is_pesticide = any(
    ...
    company_indicators = ['công ty', 'cong ty',
                          'cp', 'tnhh', 'ltd', 'co.', 'corp']
...
def import_phan_bon_smart(conn):
    ...
    for file_name in excel_files:
        ...
        try:
            ...
            for sheet_name in xls.sheet_names:
                ...
                thanh_phan_col = column_map.get(
                    'thanh_phan') or column_map.get('hoat_chat')
                ...
                for idx, row in df.iterrows():
                    ...
                    if thanh_phan_col and pd.notna(row.get(thanh_phan_col)):
                        thanh_phan = str(row.get(thanh_phan_col)).strip()[
                            :500]  # Giới hạn 500 ký tự
                    ...
                    try:
                        ...
                        if idx < 3:
                            print(
                                f"         ⚠️ Row {idx+1} lỗi: {str(e)[:60]}...")

                if imported_count > 0:
                    conn.commit()
                    print(
                        f"      ✅ Import {imported_count} phân bón từ sheet này")
    ...
    print(f"\n{'='*70}")
    print(
        f"✅ HOÀN TẤT: Import {total_imported} phân bón từ {len(excel_files)} file")
...
def import_thuoc_bvtv_smart(conn):
        ...
        try:
            ...
            for sheet_name in xls.sheet_names:
                ...
                if imported_count > 0:
                    conn.commit()
                    print(
                        f"      ✅ Import {imported_count} thuốc từ sheet này")
    ...
    print(f"\n{'='*70}")
    print(
        f"✅ HOÀN TẤT: Import {total_imported} thuốc BVTV từ {len(excel_files)} file")
