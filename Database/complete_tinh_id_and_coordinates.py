...
def build_province_mapping(provinces_dict):
    ...
    province_variants = {
        ...
        'Đắk Nông': ['dak nong', 'daknong', 'dac nong', 'gia nghia'],
...
def extract_province_advanced(address: str, ten_co_so: str = None, province_mapping: dict = None) -> str:
    ...
    combined_text = re.sub(
        r'\b(tinh|tỉnh|province|city|tp|thanh pho|thành phố|huyen|huyện|district|xa|xã|commune|ward)\b', ' ', combined_text)
...
def update_tinh_id_comprehensive(conn):
    ...
    print(
        f"   Đã có tinh_id:          {has_tinh_id:,} ({has_tinh_id/total*100:.1f}%)")
    print(
        f"   Cần cập nhật:           {needs_update:,} ({needs_update/total*100:.1f}%)")
    ...
    for fid, ma_co_so, ten_co_so, dia_chi, current_tinh_id in facilities:
        ...
        province_name = extract_province_advanced(
            dia_chi, ten_co_so, province_mapping)
        ...
        if province_name and province_name in provinces:
            ...
            if updated <= 10 or updated % 1000 == 0:
                print(
                    f"   ✅ [{updated:,}] {ma_co_so}: {ten_co_so[:40] if ten_co_so else 'N/A'}")
            ...
            if failed <= 5:
                print(
                    f"   ❌ Failed: {ma_co_so}: {ten_co_so[:40] if ten_co_so else 'N/A'}")
                print(f"      Address: {dia_chi[:60] if dia_chi else 'N/A'}")
    ...
    print(f"\n📊 TỔNG KẾT:")
    print(
        f"   Cơ sở có tinh_id:  {final_count:,}/{total:,} ({final_count/total*100:.1f}%)")
...
def generate_coordinates_for_all(conn):
    ...
    print(
        f"   Có tinh_id:       {has_province:,} ({has_province/total*100:.1f}%)")
