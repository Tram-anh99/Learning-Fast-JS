...
def clean_co_so_invalid_data(conn):
    ...
    co_so_tables = ['co_so', 'co_so_dong_goi',
                    'co_so_phan_bon', 'co_so_thuoc_bvtv']
    total_deleted = 0
...
def drop_column_ma_tinh(conn):
    ...
    try:
        cur.execute(
            "ALTER TABLE nongsan.tinh DROP COLUMN IF EXISTS ma_tinh CASCADE")
...
def move_hoat_chat_to_ghi_chu(conn):
    ...
    try:
        # Check if ghi_chu column exists, if not create it
        cur.execute("""
            SELECT column_name FROM information_schema.columns
            WHERE table_schema='nongsan' AND table_name='thuoc_bvtv' AND column_name='ghi_chu'
        """)

        if cur.fetchone() is None:
            # Add ghi_chu column first
            cur.execute(
                "ALTER TABLE nongsan.thuoc_bvtv ADD COLUMN ghi_chu TEXT")
            print("   ✅ Added ghi_chu column to thuoc_bvtv")

        # Update ghi_chu with ten_hoat_chat content where it exists
        cur.execute("""
        ...
        # Now drop the ten_hoat_chat column
        cur.execute(
            "ALTER TABLE nongsan.thuoc_bvtv DROP COLUMN IF EXISTS ten_hoat_chat")
