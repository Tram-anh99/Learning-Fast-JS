            ...
            if pd.notna(ma_vung_puc) and pd.notna(ten_vung):
                try:
                    ...
                    """, (
                        str(ma_vung_puc),
                        str(ten_vung).strip(),
                        float(row.get('dientich')) if pd.notna(
                            row.get('dientich')) else None,
                        str(row.get('nguoidaidien')) if pd.notna(
                            row.get('nguoidaidien')) else None,
                        str(row.get('xa')) if pd.notna(
                            row.get('xa')) else None,
                        str(row.get('huyen')) if pd.notna(
                            row.get('huyen')) else None,
                        str(row.get('tinh')) if pd.notna(
                            row.get('tinh')) else None
                    ))
                    imported += 1
                except Exception as e:
                    conn.rollback()
    ...
    try:
        ...
        for idx, row in df.iterrows():
            ...
            if pd.notna(cay_id) and pd.notna(ten_cay):
                ...
                ten_khoa_hoc = str(row.get('ten_khoa_hoc')) if pd.notna(
                    row.get('ten_khoa_hoc')) else None
            ...
            if pd.notna(so_bang) and pd.notna(ten_giong):
                try:
                    ...
                    if pd.notna(row.get('ngaydk_bd_hieuluc')):
                        try:
                            ngay_bd = pd.to_datetime(
                                row.get('ngaydk_bd_hieuluc')).date()
                        except:
                            pass
                    ...
                    """, (
                        str(so_bang),
                        str(row.get('giong_ID')) if pd.notna(
                            row.get('giong_ID')) else None,
                        str(ten_giong).strip(),
                        str(row.get('tenchusohuu')) if pd.notna(
                            row.get('tenchusohuu')) else None,
                        ngay_bd,
                        str(row.get('Tình trạng bằng')) if pd.notna(
                            row.get('Tình trạng bằng')) else None
                    ))
                    imported += 1
                except Exception as e:
                    conn.rollback()
...
def import_coso_directory(conn, subdir, table_name, prefix):
    ...
    excel_files = [f for f in os.listdir(
        dir_path) if f.endswith(('.xlsx', '.xls'))]
    if not excel_files:
                ...
                if pd.notna(ten_co_so) and str(ten_co_so).strip():
                    ...
                    for col in df.columns:
                        if any(keyword in str(col).lower() for keyword in ['địa chỉ', 'dia chi', 'address']):
                            dia_chi = str(row.get(col)) if pd.notna(
                                row.get(col)) else None
                            break
                    ...
                    for col in df.columns:
                        if any(keyword in str(col).lower() for keyword in ['điện thoại', 'dien thoai', 'sdt', 'phone']):
                            dien_thoai = str(row.get(col)) if pd.notna(
                                row.get(col)) else None
                            break
