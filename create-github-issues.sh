...
- ✅ Placeholder sẵn sàng cho quét camera | Placeholder for camera scan ready
...
**Bước 1:** Cài đặt package | Install package
\`\`\`bash
npm install html5-qrcode
\`\`\`

**Bước 2:** Cập nhật QRScanner.vue | Update QRScanner.vue
- Thêm container hiển thị camera | Add camera preview container
- Khởi tạo Html5Qrcode scanner | Initialize Html5Qrcode scanner
- Xử lý callback khi quét thành công | Handle successful scan callback
- Xử lý lỗi quét | Handle scan errors
- Yêu cầu quyền truy cập camera | Add camera permissions request
- Dừng camera khi đóng modal | Add camera stop on modal close

**Bước 3:** Thêm nút chuyển đổi camera | Add camera toggle button

**Bước 4:** Kiểm thử trên thiết bị di động | Test on mobile devices (iOS/Android)

---

### ✅ Tiêu chí chấp nhận | Acceptance Criteria
- [ ] Hiển thị camera preview trong modal QRScanner | Camera preview shows in QRScanner modal
- [ ] Quét thành công mã QR từ camera | Successfully scans QR codes from camera
- [ ] Tự động chuyển đến trang chi tiết sau khi quét | Auto-redirects to product details after scan
- [ ] Xử lý quyền camera một cách hợp lý | Handles camera permissions gracefully
- [ ] Hoạt động trên trình duyệt mobile (iOS Safari, Chrome Android) | Works on mobile browsers
- [ ] Có fallback về nhập thủ công nếu camera không khả dụng | Fallback to manual input if camera unavailable

---

### 🛠️ Chi tiết kỹ thuật | Technical Details

**Files cần chỉnh sửa | Files to modify:**
